...
### 2026-07-07 Advisory Pipeline Live End-to-End Run

Decisions made while validating the advisory pipeline against a live Anthropic call. The version-compatibility gotcha below was invisible to the unit tests (which use a fake `IChatClient`) and surfaced only under real model invocation.
