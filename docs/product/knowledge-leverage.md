...
Knowledge Leverage is the outward-facing capability of DEKE. Where the Knowledge Base builds and maintains trustworthy domain knowledge, Knowledge Leverage transforms that knowledge into grounded expert advisory responses. It is the interface between DEKE's accumulated domain understanding and its consumers --- whether human users, AI coding sessions, or other agents.

The base reasoning capability comes from a general-purpose language model. The domain competence comes from DEKE's accumulated knowledge. As the knowledge base deepens, the minimum capable model decreases --- the cost curve bends favourably over time.

## The Three-Layer Stack

Knowledge Leverage is organised into three layers with strictly defined responsibilities:
...
| **Layer 2** | Shared Core | Context assembly from DEKE facts, trust metadata interpretation, confidence banding, uncertainty expression language, model call mechanics, graceful degradation, and interaction logging. Built once, improves once. All domains benefit automatically. |
| **Layer 3** | Domain Adapter | The minimal, replaceable plugin: domain-specific prompt strategy, fact weighting, context formatting, and trust calibration overrides. Domain-specific logic lives here and only here. |

This separation is the most important design decision in Knowledge Leverage. Without it, every new domain requires rebuilding the entire advisory pipeline. With it, a new domain is a small adapter with a handful of domain-specific behaviours. When the language model improves, the shared core is updated and all adapters benefit. When the trust model evolves, the shared core changes and all domains inherit the improvement.

## Advisory Pipeline

Every advisory call passes through seven stages in sequence:

1. **Request validation.** Confirm the domain exists and is activated (knowledge threshold met). Validate the query.
2. **Fact retrieval.** Retrieve the most relevant facts from the Knowledge Base by semantic similarity, applying trust filters (minimum confidence).
3. **Context assembly.** The domain adapter formats the ranked facts for the prompt, translating trust metadata into natural language guidance --- for example, "multiple independent sources confirm this" or "sources disagree on this point."
4. **Prompt construction.** Combine the adapter's system prompt, trust guidance, assembled context, and the original query into the final prompt.
5. **Model call.** Dispatch the prompt to the selected language model backend. Capture token counts.
6. **Response assembly.** Wrap the model output in a structured response including confidence band, cited fact references, and identified knowledge gaps.
7. **Interaction logging.** Capture an interaction record containing the interaction identifier, cited facts, confidence band, and model used.
...
Knowledge Leverage handles three classes of "cannot answer" scenarios:

| Class | Scenario | Response Strategy |
|---|---|---|
| **Knowledge gap** | DEKE has some knowledge but not enough to ground a confident response. | Deliver a low-confidence response with explicit gap declaration. Log as a demand signal for the Knowledge Base. Never fabricate to fill the gap. |
...
| **Unknown domain** | No adapter and no knowledge exist for this domain. | Return an explicit "outside DEKE scope" response. |

## Domain Activation

DEKE does not serve advisory responses for a domain that has not met its activation criteria. A system that answers poorly is worse than one that honestly says it cannot answer yet.
...
Knowledge Leverage supports multiple language model backends. The selection policy is driven by stakes level, domain adapter preference, and knowledge depth:
...
- **Capable backend.** A more capable model used when confidence is low and stakes are high, or when the caller explicitly requests it.
...
- [architecture/specification.md](../architecture/specification.md) for architecture details, interface definitions, and phased delivery plan.
